...
const ProfileMobileTabs = () => {
   ...
   const pathname = usePathname()
    const profileTabs = [
        { title: 'mon compte', url: "/profile", icon: <FaComputer size={40} />},
  ...
  return (
    <div className="w-full overflow-x-scroll flex  gap-x-[60px]  lg:hidden px-4">
        {profileTabs.map((item,index)=> (
             <Link href={item.url} key={index} className="flex flex-col gap-1 items-center text-center ">
             <div className={`${pathname === item.url ? 'bg-[#0b4d54]' : "bg-[#ddd]"} md:w-[100px] md:h-[100px] w-[70px] h-[70px] flex items-center justify-center  rounded-full`}>
